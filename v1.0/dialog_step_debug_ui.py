    ...
    def run_debug(self, transcript: str) -> dict[str, Any]:
        ...
        try:
            if self.config.use_local_model_branch:
                ...
                raw_output = self._call_local_model(self.local_extract_system_prompt, prompt)
                parsed_output = self._parse_local_qa_output(raw_output or "")
                filtered_output = self._filter_local_qa_pairs(parsed_output, strict=True)
                final_output = self._finalize_local_qa_pairs(transcript, parsed_output, raw_output or "")
