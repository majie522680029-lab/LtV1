...
参照原始对话（仅用于核验答案中的业务限定词是否在原始对话中出现过，不能引入新信息）：
{{source_dialog_text}}
...
参照原始对话区分个案细节与业务限定词（仅核验用，不引入新信息）。个案细节（具体号码、姓名、订单号、具体金额、具体月份）需删除；业务限定词（产品名、套餐名、报错文案、功能点名、业务状态码）必须保留：
{{source_dialog_text}}
...
只输出 JSON 数组："""
    ...
    def _rewrite_pairs_local(self, qa_pairs, source_dialog_text=""):
        if not qa_pairs:
        ...
        prompt = self.local_rewrite_user_prompt.format(candidate_qa_text=candidate_qa_text, source_dialog_text=(source_dialog_text or "")[:3000])
    ...
    def _genericize_pairs_local(self, qa_pairs, source_dialog_text=""):
        if not qa_pairs:
        ...
        for qa in qa_pairs:
            ...
            prompt = self.local_generic_user_prompt.format(qa_text=qa_text, source_dialog_text=(source_dialog_text or "")[:3000])
    ...
    def extract_qa_from_transcript(self, transcript):
        ...
        if self.config.use_local_model_branch:
            ...
            rewritten_pairs = self._rewrite_pairs_local(filtered_pairs, source_dialog_text=self._current_source_dialog_text)
            final_pairs = self._genericize_pairs_local(rewritten_pairs, source_dialog_text=self._current_source_dialog_text)
            self.print_model_raw_output(
                "local_qa_extract_flow",
                "【单条对话 QA 抽取全过程】\n"
                "【最终结果】\n"
