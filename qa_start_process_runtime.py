...
提取的问答对："""
        ...
        self.local_direct_user_prompt = (
            ...
            "示例6：\n"
            "输入：\n"
            "CUSTOMER: 机顶盒提示账号不存在如何处理？\n"
            "SERVICE: 可能是系统账号异常或绑定信息错误，需核实机顶盒绑定的账号是否正常。\n"
            "输出：\n"
            "[]\n"
            "（原因：答案只有\"可能是XX，需核实\"，没有确认后的操作步骤或规则依据）\n\n"
            "示例7：\n"
            "输入：\n"
            "CUSTOMER: 宽带解约订单未生成如何处理？\n"
            "SERVICE: 可能是解约流程没走完，检查一下订单状态，确认提交了没有。没生成的话可以撤了重提一下。\n"
            "输出：\n"
            "[]\n"
            "（原因：答案只描述了\"可能是流程问题，检查、重试\"，没有形成可复用的稳定规则）\n\n"
            "示例8：\n"
            "输入：\n"
            "CUSTOMER: 用户撤单成功后款项未到账怎么处理？\n"
            "SERVICE: 需要确认是否已退款，如果系统显示已退款仍需核实到账情况，可能是账本信息没同步或者订单状态异常。\n"
            "输出：\n"
            "[]\n"
            "（原因：答案只有\"确认/核实某个可能的原因\"，没有给出具体的操作路径或判断依据）\n\n"
            "现在处理以下对话：\n{transcript}\n\n只输出 JSON 数组："
        )
...
4. QA 答案是“系统问题、同步延迟、可能存在异常、建议重新订购、需后台修复”等低确定性描述，且没有稳定规则依据。特别注意：答案仅说“可能是XX原因，需确认/核实/检查”，但没有进一步给出确认之后的具体操作步骤、判断标准或规则依据的，不能仅因“问题抽象成立”而保留——必须答案本身具备可执行性才可入库。
